...
#define APP_SHORT_NAME "VkHelloNsightAftermath"
#ifdef _WIN32
...
static char const *const tex_files[] = {"checkerboard.ppm"};
...
static std::vector<uint32_t> LoadSpirvBinary(std::string path) {
    std::ifstream spirvFile(path, std::ios::ate | std::ios::binary);
    VERIFY(spirvFile.is_open());

    size_t spirvFileSize = (size_t)spirvFile.tellg();
    VERIFY(spirvFileSize % sizeof(uint32_t) == 0);
    std::vector<uint32_t> shaderBinary(spirvFileSize / sizeof(uint32_t));
    spirvFile.seekg(0);
    spirvFile.read((char*)shaderBinary.data(), spirvFileSize);
    return shaderBinary;
}
...
void Demo::init_vk() {
    ...
    if (device_extension_count > 0) {
        ...
        for (uint32_t i = 0; i < device_extension_count; i++) {
            ...
            if (!strcmp(VK_NV_DEVICE_DIAGNOSTICS_CONFIG_EXTENSION_NAME, device_extensions[i].extensionName)) {
                diagnosticsConfigExtFound = 1;
                extension_names[enabled_extension_count++] = VK_NV_DEVICE_DIAGNOSTICS_CONFIG_EXTENSION_NAME;
            }
#endif
...
#if USE_NSIGHT_AFTERMATH
    if (!diagnosticCheckPointsExtFound) {
        ERR_EXIT("vkEnumerateInstanceExtensionProperties failed to find the " VK_NV_DEVICE_DIAGNOSTIC_CHECKPOINTS_EXTENSION_NAME
            " extension.\n\nDo you have a compatible "
            "Vulkan installable client driver (ICD) and NVIDIA display "
            "driver installed?",
            "vkCreateInstance Failure");
    }
    if (!diagnosticsConfigExtFound) {
        ERR_EXIT("vkEnumerateInstanceExtensionProperties failed to find the " VK_NV_DEVICE_DIAGNOSTICS_CONFIG_EXTENSION_NAME
            " extension.\n\nDo you have a compatible "
            "Vulkan installable client driver (ICD) and NVIDIA display "
            "driver installed?",
            "vkCreateInstance Failure");
...
void Demo::update_data_buffer() {
    ...
    const float translationSpeed = 0.050f;
    const float offsetBounds = 5.0f;

    ((vktexcube_vs_uniform*)data.value)->offset[0] += translationSpeed;
    if ((((vktexcube_vs_uniform*)data.value)->offset[0] > offsetBounds))
    {
        ((vktexcube_vs_uniform*)data.value)->offset[0] = -offsetBounds;
    }
...
#include "checkerboard.ppm.h"
bool Demo::loadTexture(const char *filename, uint8_t *rgba_data, vk::SubresourceLayout *layout, int32_t *width, int32_t *height) {
    (void)filename;
    char *cPtr;
    cPtr = (char *)checkerboard_ppm;
    if ((unsigned char *)cPtr >= (checkerboard_ppm + checkerboard_ppm_len) || strncmp(cPtr, "P6\n", 3)) {
        return false;
    }
    while (strncmp(cPtr++, "\n", 1))
        ;
    ...
    while (strncmp(cPtr++, "\n", 1))
        ;
    if ((unsigned char *)cPtr >= (checkerboard_ppm + checkerboard_ppm_len) || strncmp(cPtr, "255\n", 4)) {
        return false;
    }
    while (strncmp(cPtr++, "\n", 1))
        ;
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR pCmdLine, int nCmdShow) {
    ...
    strncpy(demo.name, "Vulkan Hello Nsight Aftermath", APP_NAME_STR_LEN);
    demo.create_window();
